...
import {
  ...
  BadTokenError,
  TokenExpiredError,
} from '../../business/helper/ApiError';
import Logger from './logger';
import { privatePemDir, publicPemDir } from '../../config';
...
export default class JWT {
  private static readPublicKey(): Promise<string> {
    return promisify(readFile)(path.join(__dirname, privatePemDir), 'utf8');
  }

  private static readPrivateKey(): Promise<string> {
    console.log(path.join(__dirname, privatePemDir));
  ...
  public static async validate(token: string): Promise<JwtPayload> {
    ...
    try {
      // @ts-ignore
      return null; //<JwtPayload>await promisify(verify)(token, cert);
    } catch (e) {
      Logger.debug(e);
  ...
  public static async decode(token: string): Promise<JwtPayload> {
    ...
    try {
      ...
      return null;
      // return <JwtPayload>(
      //   await promisify(verify)(token, cert, { ignoreExpiration: true })
      // );
    } catch (e) {
      Logger.debug(e);
      throw new BadTokenError();
